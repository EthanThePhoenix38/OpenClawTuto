#!/bin/bash
# =============================================================================
# Script de déploiement OpenClaw sur Kubernetes
# Version: 1.0.0
# Auteur: Ethan Bernier (ORCID: 0009-0008-9839-5763)
# Description: Déploie OpenClaw et ses dépendances sur k3s/k8s
# =============================================================================
#
# USAGE :
#   chmod +x deploy-openclaw.sh
#   ./deploy-openclaw.sh                    # Déploiement complet
#   ./deploy-openclaw.sh --dry-run          # Simulation
#   ./deploy-openclaw.sh --delete           # Suppression
#   ./deploy-openclaw.sh --status           # État actuel
#
# PRÉREQUIS :
...
readonly LOG_FILE="/tmp/deploy-openclaw-$(date +%Y%m%d-%H%M%S).log"
...
create_secrets_interactive() {
    ...
    if kubectl get secret openclaw-api-keys -n "$NAMESPACE" &> /dev/null; then
...
deploy() {
    ...
    log STEP "Déploiement d'OpenClaw sur Kubernetes..."
    echo ""
...
wait_for_pods() {
    ...
    # Attendre le deployment OpenClaw
    if kubectl rollout status deployment/openclaw -n "$NAMESPACE" --timeout="$TIMEOUT"; then
        log OK "Deployment openclaw prêt"
    else
        log WARN "Le deployment openclaw n'est pas prêt"
        log INFO "Vérifiez avec: kubectl logs -n openclaw -l app=openclaw"
    fi
...
show_status() {
    log INFO "État du déploiement OpenClaw"
    echo ""
    ...
    local pod_name
    pod_name=$(kubectl get pods -n "$NAMESPACE" -l app=openclaw -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")

    if [[ -n "$pod_name" ]]; then
        # Test health endpoint
        if kubectl exec -n "$NAMESPACE" "$pod_name" -- curl -s http://localhost:18789/health 2>/dev/null; then
            echo ""
            log OK "OpenClaw répond sur /health"
        else
            log WARN "OpenClaw ne répond pas"
        fi
    else
        log WARN "Aucun pod OpenClaw trouvé"
    fi
}
...
delete_deployment() {
    log WARN "Suppression du déploiement OpenClaw..."
    ...
    kubectl delete secret openclaw-api-keys -n "$NAMESPACE" --ignore-not-found
...
show_logs() {
    ...
    if [[ "$follow" == "true" ]]; then
        kubectl logs -n "$NAMESPACE" -l app=openclaw -f
    else
        kubectl logs -n "$NAMESPACE" -l app=openclaw --tail=100
    fi
}
...
main() {
    echo "=============================================="
    echo " Déploiement OpenClaw sur Kubernetes"
    echo " Version: 1.0.0"
    echo "=============================================="
    echo ""
